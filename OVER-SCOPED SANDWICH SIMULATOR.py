...
class GameScreen(QWidget):
    ...
    def update_game_data(self, data):
        ...
        self.customers_today = 0
        self.max_customers_per_day = 5 + (data['day'] // 3)
        self.current_sandwich = []
        self.current_order = []
    ...
    def show_story_popup(self, title, message, callback=None, option_a=None, option_b=None, choice_callback=None):
        ...
        # Wrap the original callback to resume game
    ...
    def generate_order(self):
        if self.customers_today >= self.max_customers_per_day:
            return

        unlocked = self.session.get('unlocked', ["Bread"])
        fillings = [i for i in unlocked if i != "Bread"]
        if not fillings: fillings = ["Bread"]
        ...
        for _ in range(num - 2):
            order.append(random.choice(fillings))
        order.append("Bread")
        
        # Item 2: Patience based on length
        self.customer_patience = 60 + (len(order) * 10)
        self.patience_bar.setMaximum(self.customer_patience)
        self.patience_bar.setValue(self.customer_patience)
        self.customer_patience_timer.start(1000)
    ...
    def setup_ingredients(self):
        ...
        unlocked_ings = list(self.session.get('unlocked', []))
        # Item 6: Sandstorm shuffles buttons
        if self.sandwich_visual.sandstorm_active:
            random.shuffle(unlocked_ings)

        for ing in unlocked_ings:
    ...
    def check_day_end(self):
        """Auto-advance day if customer limit reached."""
        if self.customers_today >= self.max_customers_per_day:
            self.log_message("<b>The shop is closing for today!</b>")
            QTimer.singleShot(1500, self.next_day)
        else:
            self.generate_order()
